...
class iResBlock(nn.Module):
    ...
    def forward(self, x, logpx=None):
        if logpx is None:
            ...
            # print("X logpx IN IRES BLOCK", x.shape, x.min(), x.max(), logpx.mean())
            g, logdetgrad = self._logdetgrad(x)
            # print("G LOG DETRAT OUT IRES BLOCK", g.mean(), logdetgrad.mean())
        ...
        with torch.enable_grad():
            ...
            if not self.exact_trace:
                ...
                if self.training and self.grad_in_forward:
                    ...
                    print("after mem eff", g.mean(), logdetgrad.mean())
                else:
                    x = x.requires_grad_(True)
                    g = self.nnet(x)
