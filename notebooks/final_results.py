...
def parse_dataframe(df: pd.DataFrame):
    ...
    df["model"] = df.apply(lambda row: row["name"].split("_")[0] if row["backbone"] != "vit" else row["model"], axis=1)
    df["model"] = df.apply(lambda row: "vit_" + row["model"] if row["backbone"] == "vit" else row["model"], axis=1)
...
def plot_sum_ranking(rank_df):
    ...
    skipna = False
    sum_rank_df["iid"] = rank_df[iid_columns].sum(axis=1, numeric_only=True, skipna=skipna)
    sum_rank_df["corruption-shift"] = rank_df[noise_columns].sum(axis=1, numeric_only=True, skipna=skipna)
    if len(in_class_columns) > 0:
        sum_rank_df["sub-class-shift"] = rank_df[in_class_columns].sum(axis=1, numeric_only=True, skipna=skipna)
    sum_rank_df["sem.-new-class-shift"] = rank_df[sem_new_class_columns].sum(axis=1, numeric_only=True, skipna=skipna)
    sum_rank_df["non-sem.-new-class-shift"] = rank_df[nonsem_new_class_columns].sum(axis=1, numeric_only=True, skipna=skipna)
    sum_rank_df = sum_rank_df.rank(na_option="keep", numeric_only=True, ascending=True)
    sum_rank_df["confid"] = rank_df.confid
    sum_rank_df["aggregated"] = sum_rank_df.sum(axis=1, numeric_only=True, skipna=skipna).rank(na_option="keep", ascending=True)
    ...
    #     axs.plot(x, y, linewidth=3.1, marker=".", ms=18, color=color_dict[sum_rank_df.confid.tolist()[ix]])
        ...
        for i,(x_, y_) in enumerate(zip(x, y)):
            ...
            if i == 0:
                ...
                arrowprops = dict(arrowstyle='-', linewidth=3.1, color=color_dict[sum_rank_df.confid.tolist()[ix]], relpos=(0, 0.5), alpha=0.4)
                xycoords = textprev
                xy = (1, 0.5)
...
#     print(axs.get_facecolor())
    ...
    axs.annotate("best\nrank", xy=(1.054, 1), xytext=(1.054, 1), xycoords='axes fraction', fontsize=16, horizontalalignment='left', verticalalignment='top')
    axs.annotate("worst\nrank", xy=(1.054, 0), xytext=(1.054, 0), xycoords='axes fraction', fontsize=16, horizontalalignment='left', verticalalignment='bottom')
    plt.tight_layout()
...
def plot_rank_style(df):
    ...
    plot_data = plot_data[plot_data.study.isin(studies1 + studies2)]
    plot_data1 = plot_data[plot_data.study.isin(studies1)]
    plot_data1 = plot_data1.groupby(["confid", "study"]).mean().reset_index()
    plot_data1 = plot_data1.sort_values(by='study', key=lambda x: x.apply(studies1.index))
    plot_data2 = plot_data[plot_data.study.isin(studies2)]
    plot_data2 = plot_data2.groupby(["confid", "study"]).mean().reset_index()
    plot_data2 = plot_data2.sort_values(by='study', key=lambda x: x.apply(studies2.index))
    scale = 10
    sns.set_style("whitegrid")
    sns.set_context("paper", font_scale=scale * 0.50)
    f, axs = plt.subplots(nrows=1, ncols=1, figsize=(4*scale, 1.5*scale * 1.2))
    x1 = np.arange(len(plot_data1.study.unique()))
    x2 = np.arange(len(plot_data1.study.unique()), len(plot_data1.study.unique()) + len(plot_data2.study.unique()))
    ranked_confids = plot_data1[plot_data1.study == studies1[0]].sort_values(by=metric, ascending=True).confid.to_list()
    logger.info(ranked_confids)
    twin1 = axs.twinx()
    # logger.info(twin1.spines)
    twin1.yaxis.tick_left()
    twin1.spines["left"].set_position(("data", len(studies1) - 1))
    for c in plot_data1["confid"]:
        confid_data1 = plot_data1[plot_data1["confid"] == c][["study", metric]].reset_index()
        confid_data2 = plot_data2[plot_data2["confid"] == c][["study", metric]].reset_index()
        axs.plot(x1, confid_data1[metric], color=color_dict[c], marker='o', linewidth=3.1, ms=18)
        twin1.plot(x2, confid_data2[metric], color=color_dict[c], marker='o', linewidth=3.1, ms=18)
        ...
        axs.annotate(
            xy=(0, confid_data1[metric].iloc[0]),
            xytext=(-0.1, ranked_confids.index(c)/(len(ranked_confids) - 1)),
            textcoords='axes fraction',
            text=c,
            fontsize=16,
            horizontalalignment="right",
            arrowprops = dict(arrowstyle='-', linewidth=3.1, color=color_dict[c], alpha=1, relpos=(1, 0.5)),
            zorder=1,
            # bbox=dict(facecolor=color_dict[c], edgecolor='None', alpha=0.5 ),
            bbox=dict(facecolor='None', edgecolor=color_dict[c], alpha=1, linewidth=3.1 ),
        )
        
    studies = list(plot_data1.study.unique()) + list(plot_data2.study.unique())
    axs.set_xticks(range(len(studies)))
    axs.set_xticklabels(studies, rotation=90)
    axs.set_xlim(-1, len(studies) - 0.5)
    ...
    axs.spines["left"].set_zorder(0.5)
