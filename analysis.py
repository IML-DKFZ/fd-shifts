...
class Analysis():
                ...
                for method_dict in self.input_list:
                    ...
                    for new_class_set in self.query_studies[study_name]:
                        for mode in ["original_mode", "proposed_mode"]:
                            ...
                            iid_set_ix = flat_test_set_list.index(self.query_studies["iid_study"][0])
                            new_class_set_ix = flat_test_set_list.index(new_class_set)
                            ...
                            if mode == "original_mode":
                                correct[select_ix_in == 0] = 1     # nice to see so visual how little practical sense the current protocol makes!
                            ...
                            bak_performance_metrics = deepcopy(self.query_performance_metrics)
                            self.query_performance_metrics = ["accuracy"]
                            self.perform_study(study_name="{}_{}_{}".format(study_name, new_class_set, mode))
                            self.query_performance_metrics = bak_performance_metrics

            if study_name == "noise_study":

                for noise_set in self.query_studies[study_name]:
                    noise_set_ix = flat_test_set_list.index(noise_set)
    ...
    def get_confidence_scores(self):
        ...
        for method_dict in self.input_list:
            ...
            if "tcp" in method_dict["query_confids"]:
                method_dict["tcp"] = {}
                # [b, cl, mcd] - [b, cl]
                method_dict["tcp"]["confids"] = external_confids
    ...
    def create_results_csv(self):
        ...
        columns = ["name", "model", "network", "fold", "confid", "n_test"] + all_metrics
...
def main(in_path=None, out_path=None, query_studies=None):
    ...
    if in_path is None: # NO SLASH AT THE END OF PATH !
        path_to_test_dir_list = [
            ...
            "/mnt/hdd2/checkpoints/checks/check_imagenet/version_39",
        ]
        # path_to_test_dir_list = [
    ...
    if query_studies is None:
        ...
        query_studies = {"iid_study": "imagenet"}
