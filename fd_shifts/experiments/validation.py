...
from omegaconf import DictConfig, OmegaConf, resolvers
...
OmegaConf.register_new_resolver("env", resolvers.oc.env)
...
def validate_experiment(experiment: Experiment):
    ...
    if config_path.is_file():
        ...
        match dconf.trainer.lr_scheduler.name:
        ...
        if hasattr(dconf.trainer, "optimizer"):
            match dconf.trainer.optimizer.name:
                case "SGD":
        ...
        # with logger.catch():
        try:
            conf: configs.Config = OmegaConf.to_object(OmegaConf.merge(schema, dconf))
            conf.validate()
            ...
            logger.error(OmegaConf.to_yaml(dconf))
            logger.error(OmegaConf.to_yaml(schema))
            raise exception
    else:
...
if __name__ == "__main__":
    ...
    fd_shifts.logger.add("{time}_validation.log", enqueue=True, level="INFO", format=format_log)
