...
class Analysis:
    def __init__(
        ...
        for path in path_list:
            ...
            if method_dict["cfg"].data.num_classes is None:
                method_dict["cfg"].data.num_classes = method_dict[
                    "cfg"
                ].trainer.num_classes
            method_dict["query_confids"] = method_dict["cfg"].eval.confidence_measures[
                "test"
            ]
        ...
        self.query_studies = (
            self.input_list[0]["cfg"].eval.query_studies
    ...
    def process_outputs(self):
        ...
        for method_dict in self.input_list:
            ...
            print(
                "CHECK IN DATASETS",
                np.unique(method_dict["raw_dataset_ix"], return_counts=True),
            )
            method_dict["raw_labels"] = raw_outputs[:, -2]
            method_dict["raw_softmax"] = raw_outputs[:, :-2]
            if "openset" in method_dict["cfg"].exp.name:
                method_dict["raw_softmax"][
                    :, method_dict["cfg"].data.kwargs.out_classes
                ] = 0
            method_dict["raw_correct"] = (
                np.argmax(method_dict["raw_softmax"], axis=1)
                == method_dict["raw_labels"]
            ) * 1
            ...
            if mcd_softmax_dist is not None:
                ...
                mcd_correct = (
                    np.argmax(mcd_softmax_mean, axis=1) == method_dict["raw_labels"]
                ) * 1
    ...
    def register_and_perform_studies(self):
        ...
        if self.add_val_tuning:
            ...
            for method_dict in self.input_list:
                ...
                select_ix = np.argwhere(method_dict["raw_dataset_ix"] == iid_set_ix)[
                    :, 0
                ]

                method_dict["study_softmax"] = deepcopy(
                    method_dict["raw_softmax"][select_ix]
                )
                method_dict["study_labels"] = deepcopy(
                    method_dict["raw_labels"][select_ix]
                )
                method_dict["study_correct"] = deepcopy(
                    method_dict["raw_correct"][select_ix]
                )

                if method_dict.get("raw_external_confids") is not None:
                    method_dict["study_external_confids"] = deepcopy(
                        method_dict["raw_external_confids"][select_ix]
                    )
                if method_dict.get("raw_external_confids_dist") is not None:
                    method_dict["study_external_confids_dist"] = deepcopy(
                        method_dict["raw_external_confids_dist"][select_ix]
                    )
                    std = method_dict["study_external_confids_dist"].std(1)
                    print(
                        "CHECK EXT DIST",
                        method_dict["study_external_confids_dist"].shape,
                        std.mean(),
                        std.min(),
                        std.max(),
                        method_dict["study_external_confids_dist"][0],
                    )

                if method_dict.get("raw_mcd_softmax_dist") is not None:
                    method_dict["study_mcd_softmax_mean"] = deepcopy(
                        method_dict["raw_mcd_softmax_mean"][select_ix]
                    )
                    method_dict["study_mcd_softmax_dist"] = deepcopy(
                        method_dict["raw_mcd_softmax_dist"][select_ix]
                    )
                    method_dict["study_mcd_correct"] = deepcopy(
                        method_dict["raw_mcd_correct"][select_ix]
                    )
            ...
            if study_name == "iid_study":
                ...
                for method_dict in self.input_list:
                    ...
                    select_ix = np.argwhere(
                        method_dict["raw_dataset_ix"] == iid_set_ix + val_ix_shift
                    )[:, 0]
            ...
            if study_name == "in_class_study":
                ...
                for method_dict in self.input_list:
                    for in_class_set in self.query_studies[study_name]:
                        ...
                        select_ix = np.argwhere(
                            method_dict["raw_dataset_ix"] == iid_set_ix
                        )[:, 0]
                    ...
                    for new_class_set in self.query_studies[study_name]:
                        for mode in ["original_mode", "proposed_mode"]:
                            ...
                            select_ix_out = np.argwhere(
                                method_dict["raw_dataset_ix"] == new_class_set_ix
                            )[:, 0]
                            ...
                            if method_dict.get("raw_mcd_softmax_dist") is not None:
                                ...
                                method_dict["study_mcd_softmax_mean"] = deepcopy(
                                    method_dict["raw_mcd_softmax_mean"][select_ix_all]
                                )
                                method_dict["study_mcd_softmax_dist"] = deepcopy(
                                    method_dict["raw_mcd_softmax_dist"][select_ix_all]
                                )
                                method_dict["study_mcd_correct"] = correct[
                                    select_ix_all
                                ]
                                if (
                                    method_dict.get("raw_external_confids_dist")
                                    is not None
                                ):
                                    method_dict[
                                        "study_external_confids_dist"
                                    ] = deepcopy(
                                        method_dict["raw_external_confids_dist"][
                                            select_ix_all
                                        ]
                                    )
                ...
                for noise_set in self.query_studies[study_name]:
                    ...
                    for intensity_level in range(5):
                        ...
                        for method_dict in self.input_list:
                            ...
                            select_ix = np.argwhere(
                                method_dict["raw_dataset_ix"] == noise_set_ix
                            )[:, 0]
                            ...
                            method_dict["study_softmax"] = (
                                deepcopy(method_dict["raw_softmax"][select_ix])
                                .reshape(
                                    15, 5, -1, method_dict["raw_softmax"].shape[-1]
                                )[:, intensity_level]
                                .reshape(-1, method_dict["raw_softmax"].shape[-1])
                            )
                            method_dict["study_labels"] = (
                                deepcopy(method_dict["raw_labels"][select_ix])
                                .reshape(15, 5, -1)[:, intensity_level]
                                .reshape(-1)
                            )
                            method_dict["study_correct"] = (
                                deepcopy(method_dict["raw_correct"][select_ix])
                                .reshape(15, 5, -1)[:, intensity_level]
                                .reshape(-1)
                            )
                            self.dummy_noise_ixs = (
                                np.arange(len(method_dict["raw_correct"][select_ix]))
                                .reshape(15, 5, -1)[:, intensity_level]
                                .reshape(-1)
                            )
                            if method_dict.get("raw_external_confids") is not None:
                                method_dict["study_external_confids"] = (
                                    deepcopy(
                                        method_dict["raw_external_confids"][select_ix]
                                    )
                                    .reshape(15, 5, -1)[:, intensity_level]
                            ...
                            if method_dict.get("raw_external_confids_dist") is not None:
                                ...
                                method_dict["study_external_confids_dist"] = (
                                    deepcopy(
                                        method_dict["raw_external_confids_dist"][
                                            select_ix
                                        ]
                                    ...
                                    .reshape(
                                        ...
                                        5,
                                        -1,
                                        method_dict["raw_external_confids_dist"].shape[
                                            -1
                                        ],
                                    )[:, intensity_level]
                                    .reshape(
                            ...
                            if any(
                                "mcd" in cfd for cfd in method_dict["query_confids"]
                            ):
                                method_dict["study_mcd_softmax_mean"] = (
                                    deepcopy(
                                        method_dict["raw_mcd_softmax_mean"][select_ix]
                                    )
                                    .reshape(
                                        ...
                                        5,
                                        -1,
                                        method_dict["raw_mcd_softmax_mean"].shape[-1],
                                    )[:, intensity_level]
                                    .reshape(
                                        -1,
                                        method_dict["raw_mcd_softmax_mean"].shape[-1],
                                ...
                                method_dict["study_mcd_softmax_dist"] = (
                                    deepcopy(
                                        method_dict["raw_mcd_softmax_dist"][select_ix]
                                    )
                                    .reshape(
                                        ...
                                        5,
                                        -1,
                                        method_dict["raw_mcd_softmax_dist"].shape[-2],
                                        method_dict["raw_mcd_softmax_dist"].shape[-1],
                                    )[:, intensity_level]
                                    .reshape(
                                        -1,
                                        method_dict["raw_mcd_softmax_dist"].shape[-2],
                                ...
                                method_dict["study_mcd_correct"] = (
                                    deepcopy(method_dict["raw_mcd_correct"][select_ix])
                                    .reshape(15, 5, -1)[:, intensity_level]
                                    .reshape(-1)
                                )
        ...
        for method_dict in self.input_list:
            ...
            if any("mcd" in cfd for cfd in method_dict["query_confids"]):
                ...
                mcd_performance_metrics = self.compute_performance_metrics(
                    mcd_softmax_mean, labels, mcd_correct, method_dict
                )

            performance_metrics = self.compute_performance_metrics(
                softmax, labels, correct, method_dict
            )
            ...
            if "det_mcp" in method_dict["query_confids"]:
                ...
                print(
                    "CHECK DET MCP",
                    np.sum(method_dict["det_mcp"]["confids"]),
                    np.sum(method_dict["det_mcp"]["correct"]),
                )

            if "det_pe" in method_dict["query_confids"]:
                method_dict["det_pe"] = {}
                method_dict["det_pe"]["confids"] = np.sum(
            ...
            if "mcd_mi" in method_dict["query_confids"]:
                method_dict["mcd_mi"] = {}
                tmp_confids = (
                    method_dict["mcd_pe"]["confids"] - method_dict["mcd_ee"]["confids"]
                )
            ...
            if any(
                cfd in method_dict["query_confids"]
                for cfd in [
                    "ext_waic",
                    "bpd_waic",
                    "maha_waic",
                ...
                tmp_confids = np.mean(
                    method_dict["study_external_confids_dist"], axis=1
                ) - np.std(method_dict["study_external_confids_dist"], axis=1)
                ...
                method_dict["query_confids"] = [
                    out_name if v == "ext_waic" else v
                    for v in method_dict["query_confids"]
                ]
            ...
            if any(
                ...
                method_dict[ext_confid_name]["confids"] = method_dict[
    ...
    def create_results_csv(self):
        ...
        for method_dict in self.input_list:
            ...
            for confid_key in method_dict["query_confids"]:
                submit_list = [
                    ...
                    confid_key,
                    method_dict["study_mcd_softmax_mean"].shape[0]
                    if "mcd" in confid_key
                    else method_dict["study_softmax"].shape[0],
                ]
                submit_list += [
                    method_dict[confid_key]["metrics"][x] for x in all_metrics
                ]
        ...
        group_file_path = os.path.join(
            self.input_list[0]["cfg"].exp.group_dir, "group_analysis_metrics.csv"
        )
    ...
    def create_master_plot(self):
        # get overall with one dict per compared_method (i.e confid)
        input_dict = {
            "{}_{}".format(method_dict["name"], k): method_dict[k]
            for method_dict in self.input_list
            for k in method_dict["query_confids"]
        }
...
def main(
    ...
    print(
        "starting analysis with in_path(s) {}, out_path {}, and query studies {}".format(
            path_to_test_dir_list, analysis_out_dir, query_studies
        )
    )

    analysis = Analysis(
        path_list=path_to_test_dir_list,
