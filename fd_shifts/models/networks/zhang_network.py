...
class ZhangNet(nn.Module):
    def __init__(self, cf):
        ...
        self.residual_flow = ResidualFlow(
            ...
            init_layer=layers.LogitTransform(
                0.05
            ),  # This is the logitTransform! So no Squeeze! alpha is 0.05 or 1e-5 for mnist! See RealNVP paper
            actnorm=True,
            fc_actnorm=True,
            batchnorm=False,
            dropout=0,
    ...
    def forward(self, x):
        ...
        z, delta_logp = self.residual_flow(
            x, 0
        )  # logpx=0 means we want to compute logpx later,
        ...
        norm = (
            np.log(self.nvals)
            * (x.size()[-1] * x.size()[-1] * (x.size()[1] + self.padding))
            - logpu
        )  # todo weird, ijust get an extra norm here for free via nvals?!
